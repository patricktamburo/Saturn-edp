...
corsnumber = '0106'
body = 'Saturn'
allrsrpaths = file_search(rootpath+'Output/'+body+'/cors_'+corsnumber+'/*')

allrsrpaths = allrsrpaths[3]
stop
...
;;;
; NOTE - WE SHOULD VERIFY THAT SFDU TIME IS A UTC TIME, NOT SOMETHING ELSE. Verified in RSR header.
;;;
...
cspice_subsol, method, targetstr, et, abcorr, earthstr, targetsubsolpt
...
cspice_reclat, targetsubsolpt, targetsubsolradius, targetsubsollongitude, targetsubsollatitude
; Convert targetsubsolpt from cartesian coordinates to r/lat/lon coordinates (km/radE/radN)
targetsubsollongitude = targetsubsollongitude *180./!pi
targetsubsollatitude = targetsubsollatitude *180./!pi
; Convert angles from radians into degrees
;print, 'targetsubsolr/lon/lat = ', targetsubsolradius, targetsubsollongitude, targetsubsollatitude

cspice_subpt, method, targetstr, et, abcorr, sctstr, subsctpoint, altsctwrttarget
; subsctpoint is position of sub-spacecraft point on target body at time et
; See help pages for frame in which targetsubsolpt is stated
; altsctwrttarget is altitude of spacecraft above sub-spacecraft point on target body at time et, km
;print, 'subsctpoint = ', subsctpoint
;print, 'altsctwrttarget = ', altsctwrttarget

cspice_reclat, subsctpoint, subsctradius, subsctlongitude, subsctlatitude
; Convert subsctpoint from cartesian coordinates to r/lat/lon coordinates (km/radE/radN)
subsctlongitude = subsctlongitude *double(180.)/!pi
subsctlatitude = subsctlatitude *double(180.)/!pi
; Convert angles from radians into degrees
;print, 'subsctr/lon/lat = ', subsctradius, subsctlongitude, subsctlatitude

cspice_spkgps, sctint, et, frame, earthint, sctearth, ltimesctearth
cspice_spkgps, targetint, et, frame, earthint, targetearth, ltimetargetearth
; ltimetargetearth is light time between target and Earth
; Useful for converting et values to UTC ERT Earth received times
; Should probably be worrying about whether et is when signal transmitted from sct or passing by target
; and a host of other items

cspice_spkezr, sctstr, et, frame, 'NONE', earthstr, state, otherltime


cspice_spkgps, targetint, et, frame, ssbint, targetssb, ltimetargetssb
; targetssb is position of target body relative to solar system barycentre at time et
; Position given in specified frame (frame, probably J2000)
cspice_spkgps, sctint, et, frame, ssbint, sctssb, ltimemgsssb
cspice_spkgps, earthint, et, frame, ssbint, earthssb, ltimeearthssb
; Positions of spacecraft and Earth at time et, 
; also relative to solar system barycentre and in specified frame
...
cspice_tipbod, frame, targetint, et, tipm
...
print, i, nsteps, occptradius, (et-etstart)/(etend-etstart), dt
...
;cspice_et2utc, real_et, 'C', 6, real_ertutc

; save, filename=path+'tests07routput.sav', real_ertutc, occptradiusarray
save, filename=path+'full_output/simpleocc_output_pat_old.sav', real_ertutc, occptradiusarray
