pro oblate_pipeline_freqwork, itloop, body, flyby, obs, bands, station
  ;it loop:
  ...
  ;If loop a, output the start/stop times to be restored in simpleocc loop 1.
  if itloop eq 'a' then begin
    start_time_save = keeptrackofutcrx[0]
    stop_time_save = keeptrackofutcrx[n_elements(keeptrackofutcrx)-1]
    save,start_time_save,stop_time_save,filename=rootpath+body+'\cors_'+xcors+'\'+xproductid+'\full_output\freqwork_a_output.sav'
    if bands eq 'XK' then save,start_time_save,stop_time_save,filename=rootpath+body+'\cors_'+kcors+'\'+kproductid+'\full_output\freqwork_a_output.sav'
    if bands eq 'SX' then save,start_time_save,start_time_save,stop_time_save,filename=rootpath+body+'\cors_'+scors+'\'+sproductid+'\full_output\freqwork_a_output.sav'
  endif
  ...
  if itloop eq 'a' then begin
    ...
    if itloop eq 'b' then begin
      restore, rootpath+body+'\cors_'+xcors+'\'+xproductid+'\' + 'pathlengthr.sav'
      restore, rootpath+body+'\cors_'+xcors+'\'+xproductid+'\' + 'pathlengthsph.sav'
      restore, rootpath+body+'\cors_'+xcors+'\'+xproductid+'\' + 'pathlengthell.sav'
      restore, rootpath+body+'\cors_'+xcors+'\'+xproductid+'\' + 'pathlengthash.sav'
      restore, rootpath+body+'\cors_'+xcors+'\'+xproductid+'\' + 'pathlengthkos.sav'
    ...
    ;Reverse order for consistency with how radius information is output.
    r_minus_R = reverse(r_minus_R)
    if strmatch(obs, '*X')then electrondensity = reverse(electrondensity)
    
    for i = 0, n_elements(thisrkm)- 1 do begin
      line = string(thisrkm[i])+', '+string(r_minus_R[i])+', '+string(electrondensity[i]/1e6)+', '+string(electrondensity_rms[i])
      printf, lun, line
    endfor
    close, lun
    free_lun, lun
    ...
    save, filename=sav_filename, ettxarraysav, etoccptarraysav, etrxarraysav, utctxarraysav, utcoccptarraysav, utcrxarraysav, $
      ...
      dxdt_save, dxdt, newbigx, edpsav, electrondensity_errorsav
