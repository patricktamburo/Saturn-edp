...
#v3 uses the A-S 1-bar surface level output from  freqwork
...
reps = ['ellipsoidal','koskinen','schubert']
representation = reps[2]
...
for i in range(size(obs_list)):
	...
	ed = idl_data.edpsav
	r_AS = idl_data.R_AS_ARRAYsav
	r_K = idl_data.R_K_ARRAYsav
	...
	elif representation == 'schubert':
		#thisrkm to Saturn altitude
		r_minus_R = alt - r_AS
		rep_point_loc = np.where(abs(r_minus_R-2000) == np.min(abs(r_minus_R-2000)))[0]
		alt -= r_AS[rep_point_loc]
	elif representation == 'koskinen':
		r_minus_R = alt - r_K
		rep_point_loc = np.where(abs(r_minus_R-2000) == np.min(abs(r_minus_R-2000)))[0]
		alt -= r_K[rep_point_loc]
	else:
...
#				all_sep[0], all_eps[0], alt+2575., ed_avg, err_avg),pckl)
		if representation == 'ellipsoidal':
			...
			pickle.dump((all_ettx[0], all_etoccpt[0], all_etrx[0], all_utctx[0], \
						 all_utcoccpt[0], all_utcrx[0], all_lat[0], all_lon[0], all_sza[0], all_lst[0], \
						 all_sep[0], all_eps[0], alt + r_AS[rep_point_loc], ed_avg, err_avg, rep_point_loc, r_AS),
						pckl)
		elif representation == 'koskinen':
			pickle.dump((all_ettx[0], all_etoccpt[0], all_etrx[0], all_utctx[0], \
						 all_utcoccpt[0], all_utcrx[0], all_lat[0], all_lon[0], all_sza[0], all_lst[0], \
						 all_sep[0], all_eps[0], alt + r_K[rep_point_loc], ed_avg, err_avg, rep_point_loc, r_K),
						pckl)
	pckl.close()
