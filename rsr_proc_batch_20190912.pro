...
;     It is meant to be run on batch jobs on the SCC only.
;     With the exception of the data file structure, it is
;     identical to "rsr_proc_single.pro".
;Calling Sequence:
;     rsr_proc_batch
;Inputs:
...
;----------------------------------------START USER INPUTS----------------------------------------;
;Is this going to be run on the SCC? 1=True, 0=False. Batch must always be 1.
SCC_mode = 0
...
;Access the arguments sent in through command line. They're a list of strings.
;commandargs = command_line_args()
body = 'Saturn'       ;i.e., Titan, Saturn, Rings, Enceladus
cors = '110'         ;e.g., 207. No zero in front. 125 is for S14N SX14, 232 for S70N K25, 233 for S70N X25
band = 'X'           ;i.e., X, S, or K. Must be capitalized.

;rsrfilesuffix = 's14sroi2005248_1323nnnx14rd.2a2'

;rsrfilesuffix = 's41saoi2008153_2120nnnx25rd.1a2'

;rsrfilesuffix = 's41saoi2008153_2120nnnx26rd.2a2'

;rsrfilesuffix = 's41saoi2008153_2120nnnk25rd.1b2'

;rsrfilesuffix = 's41saoi2008153_2120nnnk26rd.2b2'

;rsrfilesuffix = 's43sroi2008239_1410nnns63rd.1b2'

rsrfilesuffix = 's11sroi2005141_0755nnnx34rd.1a2'

datapath = 'C:\Users\tambu\Documents\BU\Science Projects\SaturnEDP\PD_MBP_files\Output\Saturn\cors_0'+cors+'\'

;datapath = '/projectnb/cassini/Cassini_Ionosphere/Data/cors_0'+cors+'/'
outputpath = datapath + rsrfilesuffix+'_x\'
;outputpath = '/projectnb/cassini/Cassini_Ionosphere/test_folder/'

;Make the output path, although it may already exist. If so, it will not overwrite the directory.
;spawn, 'mkdir '+outputpath

;Choose the RSR file with the desired band and polarization
rsrfile = datapath+rsrfilesuffix
...
;nforfft = 16000L ;we now define this based off of the reported sample rate in the lbl file. 
...
while not EOF(lun0) do begin
  ...
  if strmatch(line,'*ROW_BYTES*') then row_bytes = long((strsplit(line,'=',/extract))(-1))
...
two_k_sample_rate_flag = 0 ;Initialize this flag to zero.

if row_bytes eq 8260 then begin ;Dealing with different sample rates for some mid-2008 Saturn occs. 
  samplewords = 2000
  nwords = 2000
  ;nforfft = 2000
  print, '2000 sample rate detected.'
  wait, 2
  two_k_sample_rate_flag = 1 ;If this is set to 1, the data will be processed at a sample rate of 2000, and 
  ; time will be forced to be referenced at the half-second (necessary for 15 Saturn occs in mid-2008, see PDS
  ; user's guide). 
endif

if (row_bytes ne 8260) and (row_bytes ne 16260) then stop ;Stop if neither of these sample rates are detected.
...
;Read in files, verify that sfdu times match what is in the .lbl files. 
;Manually add more rows to test if there's inconsistency between label and content. 
;Verify that timespan of data matches timespan in label and...
; see what occptrad values correspond to those times. 
;This demonstrates (or not) that time series don't cross ionosphere. 
STOP

rf_point_1_arr = dblarr(nrows)
...
while row lt nrows do begin
  ...
  rf_point_1_arr(row) = swap_endian(col55)
...
;Define the sample rate from the array, assuming the sample rate check has been passed. Note the unit of kHz.
samplerate = sfdusampleratearr[0]*1000D
nforfft = samplerate ;Force 1-second time resolution
...
stop
;if sfdusampleratearr[0] ne 16 then stop ;CHECK IF EVERYTHING IS OKAY, THEN CONTINUE IF DIFFERENT SAMPLING RATE, E.G. 2
...
while i lt iend do begin 
  ...
  ;if in a 2000-samples-per-second observation, then every index above goes to [i]. Force time to be referenced
  ; to the half-second
  if two_k_sample_rate_flag then begin
    sfduyearout[kindex] = sfduyear(i)
    sfdudoyout[kindex] = sfdudoy(i)
    sfdusecout[kindex] = sfdusec(i) + 0.5D
    rftoifmhzout[kindex] = rftoifmhzarr(i)
    ddclomhzout[kindex] = ddclomhzarr(i)
  endif
  ;
  ...
  ;if in a bad obs., then every index above goes to [i]
  if two_k_sample_rate_flag then begin
    thisf1 = f1arr(i)
    thisf2 = f2arr(i) ; calculate F1, F2, F3 at X.000 seconds, so F2 and F3 irrelevant
    thisf3 = f3arr(i)
  endif
...
;save, filename=outputpath+'output.sav', sfduyearout, sfdudoyout, sfdusecout, rftoifmhzout, ddclomhzout, ncofreqout, kposaaout, igcomplexaaout
save, filename=outputpath+'test_output.sav', sfduyearout, sfdudoyout, sfdusecout, rftoifmhzout, ddclomhzout, ncofreqout, kposaaout, igcomplexaaout
